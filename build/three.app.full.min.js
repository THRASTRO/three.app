...
this.trigger("decrease");this.invoke("deleted",b);this.trigger("resized")}).method("add",function(a){if(this.belowSoftLimit())this.items.push(a),this.length+=1,this.trigger("increase");else if(this.belowHardLimit())this.items.push(a),this.options.softLimit*=this.options.increment||2,this.options.softLimit>this.options.hardLimit&&(this.options.softLimit=this.options.hardLimit),this.length+=1,this.invoke("resize");else throw Error("group reached hard limit");this.invoke("inserted",a);this.trigger("resized")});
...
(function(a,d){function c(a){var b=this.context,c=this.options;b.font=(a.fontSize||c.fontSize)+"px "+(a.fontFamily||c.fontFamily);b.fillStyle=a.color||c.fillStyle;b.strokeStyle=a.stroke||c.strokeStyle;b.lineWidth=a.lineWidth||c.lineWidth}function b(a){var b=a.fit,c=this.context;a.background&&(c.fillStyle=a.background,c.fillRect(b.x,b.y,a.w,a.h));a.border&&(c.strokeStyle=a.border,c.rect(b.x,b.y,a.w,a.h),c.stroke())}function e(a){var b=a.fit,c=b.x+2,b=b.y+a.h,b=b-2-2,d=this.context,e=this.options;(a.stroke||
e.strokeStyle)&&d.strokeText(a.txt,c,b);(a.color||e.fillStyle)&&d.fillText(a.txt,c,b)}function f(a){a--;a|=a>>1;a|=a>>2;a|=a>>4;a|=a>>8;a|=a>>16;a++;return a}var h=new GrowingPacker;a=d.Class.create("Labels",null,["Group"]).defaults({hardLimit:8192,fontSize:10,fontFamily:"Tahoma, sans",strokeStyle:"#0F0F0F",lineWidth:1}).ctor(function(a){this.canvas=document.createElement("canvas");this.context=this.canvas.getContext("2d");a.viewport.appendChild(this.canvas);this.canvas.style.position="absolute";
this.canvas.style.display="none";this.canvas.style.left="0px";this.canvas.style.top="0px";this.canvas.height=128;this.canvas.width=128}).listen("resized",function(){this.app.log("drawing texture");this.items.sort(function(a,b){return Math.max(b.w,b.h)-Math.max(a.w,a.h)});for(var a=0;a<this.length;a+=1)delete this.items[a].fit;h.fit(this.items);var a=h.root,d=this.canvas,p=this.context;d.width=f(a.w);d.height=f(a.h);p.clearRect(0,0,d.width,d.height);for(a=0;a<this.length;a+=1)b.call(this,this.items[a]),
c.call(this,this.items[a]),e.call(this,this.items[a]);this.invoke("texture.drawn")}).listen("inserted",function(a){var b=this.context,d=this.options;c.call(this,a);a.h=2+(a.fontSize||d.fontSize)+2;b=b.measureText(a.txt);a.w=2+b.width+2});d("Labels",a)})(THREE,THREEAPP);
...
h=0;h<this.items.length;h++){var g=this.items[h];d[4*h+0]=g.fit.x+2;d[4*h+1]=g.fit.y+2;d[4*h+2]=g.w-2;d[4*h+3]=g.h-2}this.geometry.attributes.offset.needsUpdate=!0;this.geometry.attributes.uvs.needsUpdate=!0;this.material.uniforms.texWidth.value=a;this.material.uniforms.texHeight.value=c;this.geometry.maxInstancedCount=this.items.length}}).listen("texture.drawn",function(){this.app.log("set texture needsUpdate");this.texture&&(this.texture.needsUpdate=!0)});d("Objects.Labels",c)})(THREE,THREEAPP);
