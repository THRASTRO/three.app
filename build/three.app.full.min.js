...
"0px";this.canvas.style.top="0px";this.canvas.height=128;this.canvas.width=128}).listen("resized",function(){this.app.log("drawing texture");this.items.sort(function(a,b){return Math.max(b.w,b.h)-Math.max(a.w,a.h)});for(var a=0;a<this.length;a+=1)delete this.items[a].fit;h.fit(this.items);var a=h.root,d=this.canvas,p=this.context;d.width=f(a.w);d.height=f(a.h);p.clearRect(0,0,d.width,d.height);for(a=0;a<this.length;a+=1)b.call(this,this.items[a]),c.call(this,this.items[a]),e.call(this,this.items[a]);
this.invoke("texture.drawn")}).listen("inserted",function(a){var b=this.context,d=this.options;c.call(this,a);a.h=2+d.fontSize+2;b=b.measureText(a.txt);a.w=2+b.width+2});d("Labels",a)})(THREE,THREEAPP);(function(a,d){a=d.Class.create("Labels",null,["Plugin"]).proto("provides","labels").ctor(function(a){a.labels=new d.Labels(a)});d("Plugin.Labels",a)})(THREE,THREEAPP);
